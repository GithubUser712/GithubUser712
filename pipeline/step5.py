...
class PurePursuit:
    """Geometric path tracker + proportional speed controller."""

    def __init__(self, xy: np.ndarray, speeds: np.ndarray, params: CarParams,
                 lookahead_gain: float = 0.30, lookahead_min: float = 0.4,
                 lookahead_max: float = 1.5, speed_kp: float = 3.0):
        self.xy = xy
        self.speeds = speeds
        self.p = params
    ...
    def reset(self, position: np.ndarray) -> None:
        self._last_idx = int(np.argmin(
            np.linalg.norm(self.xy - position, axis=1)))
    ...
    def control(self, state) -> tuple[np.ndarray, float, float]:
        """-> (env action [steer, throttle], cross-track error, target speed)"""
        ...
        accel = float(np.clip(accel, -self.p.max_brake_mps2,
                              self.p.max_accel_mps2))
        throttle = (accel / self.p.max_accel_mps2 if accel >= 0.0
